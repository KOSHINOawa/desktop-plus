  ...
  public render() {
    return (
      ...
      >
        <DialogContent>
          <p>
            本 GitHub 开源应用程序条款与条件（"应用程序条款"）是你（作为个人或以实体名义）与
          ...
          <p>
            ...
            GitHub.com 网站服务条款及/或适用于你的 GitHub Enterprise 实例的许可协议（"GitHub
            条款"）约束。
          </p>

          <p>
            任何违反你适用的 GitHub 条款而使用本软件的行为，也将构成对这些应用程序条款的违反。
          </p>
          ...
          <p>
            随软件一起授予的许可并不涵盖 GitHub 的商标，其中包括软件标识设计。GitHub
            保留对所有 GitHub 商标的全部商标权与著作权。GitHub
            的标识包括但不限于"标识"文件夹中文件名带有"logo"的程式化设计。
          </p>

          <p>
            GitHub、GitHub Desktop、GitHub for Mac、GitHub for Windows、Atom、Octocat
            以及相关的 GitHub 标识和/或程式化名称均为 GitHub
            的商标。你同意未经 GitHub
            事先书面许可，不以任何方式展示或使用这些商标，但 GitHub
            标识与使用政策允许的除外：{' '}
            <LinkButton uri={logos}>标识</LinkButton>。
          </p>
          ...
          <p>
            本软件可能包含自动更新服务（"服务"）。如果你选择使用该服务，或下载了自动启用该服务的软件，当有新版本可用时，GitHub 将自动更新该软件。
          </p>
          ...
          <p>
            ...
            不保证：(i) 服务将满足你的特定需求；(ii) 服务与任何特定平台完全兼容；(iii)
            你对服务的使用将是不间断、及时、安全或无差错的；(iv) 通过使用服务可能获得的结果将是准确或可靠的；(v)
            你通过服务购买或获取的任何产品、服务、信息或其他材料的质量将符合你的预期；或 (vi) 服务中的任何错误将被纠正。
          </p>

          <p>
            你明确理解并同意，对于因服务引起的任何直接、间接、偶然、特殊、后果性或惩罚性损害，包括但不限于利润、商誉、使用价值、数据或其他无形损失的损害（即使
            GitHub 已被告知发生此类损害的可能性），GitHub
            概不负责，例如：(i) 使用或无法使用服务；(ii) 因通过服务购买或获取任何商品、数据、信息或服务，或收发消息或进行交易，而产生的替代商品与服务的采购成本；(iii)
            对你传输内容或数据的未授权访问或篡改；(iv) 服务上任何第三方的陈述或行为；(v) 或与服务有关的任何其他事项。
          </p>
          ...
          <ol>
            <li>
              不放弃权利。GitHub 未行使或执行这些应用程序条款中的任何权利或规定，不构成对该权利或规定的放弃。
            </li>

            <li>
              完整协议。这些应用程序条款连同任何适用的隐私声明，构成你与
              GitHub 之间的完整协议，并管辖你对软件的使用，取代你与 GitHub
            ...
            <li>
              ...
              授权代表签署的书面修订，或由 GitHub 发布修订版本进行修改。这些应用程序条款连同任何适用的开源许可证与声明以及
              GitHub 隐私声明，构成你与我们之间协议的完整且唯一的陈述。这些应用程序条款取代任何口头或书面的提议、先前的协议，以及你与
