...
* Run `pnpm` to get all required dependencies on your machine.
* Run `pnpm build:dev` to create a development build of the app.
* Run `pnpm start` to launch the application. Changes will be compiled in the
...
```shellsession
$ screen -S "desktop" pnpm start # -S sets the name of the session; you can pick anything
...
If you've made changes in the `main-process` folder you need to run `pnpm
build:dev` to rebuild the package, and then `pnpm start` for these changes to be
...
- `pnpm test:script` - Runs all script tests
- `pnpm test:eslint` - Runs all eslint tests 
...
1. Run the command `pnpm start` to launch the app
